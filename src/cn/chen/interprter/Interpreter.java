...
/**
 * C0语言的解释程序s
 * Created by chen on 2016/10/26 0026.
 */
            ...
            switch (currentCode.operator.toString()) {
                ...
                case "CAL"://CAL 0 a 调用函数，a为函数地址
                    if(i==0){
                        ...
                        runStack[top_Addr + 1] = 0; //当前指令地址入栈 RA(用作返回地址)
                        i++;
                    }else{
                        runStack[top_Addr] = base_Addr;        //当前基址入栈(用作动态链）
                ...
                case "RED"://RED 0 0 从命令行读入一个输入置于栈顶
                    ...
                    while (flag) {
                        try {
                            runStack[top_Addr] = Integer.parseInt(sc.nextLine());//只能输入数字
                            flag = false;
                        } catch (Exception e) {
                ...
                case "WRT"://WRT 0 0 栈顶值输出至屏幕并换行
                    top_Addr--;
                    System.out.println(runStack[top_Addr]);
                    break;
